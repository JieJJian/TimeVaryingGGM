...
for (data.num in 1){
  ...
  for (i in 1:bs.k) {
    xi[[i]] <- mvrnorm(m,rep(0,p),xi_cov[[i]],empirical = TRUE) # need to check
  }
  ...
  for (lam1 in 1:length(se1) ){
    for (lam2 in 1:length(se2) ){
      ind=ind+1
      GFL_result <- TVNet_GFL_ADMM(YY,se1[lam1],se2[lam2],alpha=10,tol_rho=1e-8,tol_sigma=1e-8,tol_admm=1e-8)
      cat("\n","(l1,l2)=(",se1[lam1],",",se2[lam2],") - time: ",as.character(Sys.time()), sep = "")
      result[[ind]]=GFL_result
    }
  }
}
save(result,file = "s1_gfl_50.RData")
t.end <- Sys.time()
print(t.end-t.start)