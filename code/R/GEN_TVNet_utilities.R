...
#
# ADMM algorithm
ADMM_block <- function(Xlist,Alist,Ylist,alpha,tol,lamb1,lamb2){
  ...
  rho_new <- admm_x_update(Alist,longVec,lamb2)
  z_new <- soft_th_vector(ratio,(u+rho_new))
  u <- rho_new-z_new
...
# the first step in the admm iteration
admm_x_update <- function(AList,longVec,lamb2){
  l <- nrow(AList[[1]])
  k <- length(AList)
  ...
  if (lamb2!=0){
    # step 1
    for (i in 1:(k-1)){
      longVec[(1+i*l):((i+1)*l)]<-longVec[(1+i*l):((i+1)*l)]+AList[[i]]%*%longVec[(1+l*(i-1)):(l*i)]
    }
    
    # step 2
    for (i in k:2){
      longVec[(1+l*(i-2)):((i-1)*l)]<-longVec[(1+l*(i-2)):((i-1)*l)]+AList[[i]]%*%longVec[(1+l*(i-1)):(l*i)]
    }
    
    # step 3
    for (i in 1:k){
      longVec[(1+l*(i-1)):(l*i)]<-AList[[i]]%*%longVec[(1+l*(i-1)):(l*i)]
    }
    
    longVec <- longVec/(2*lamb2)
  } else {
    for (i in 1:k){
      longVec[(1+l*(i-1)):(l*i)]<-AList[[i]]%*%longVec[(1+l*(i-1)):(l*i)]
    }
